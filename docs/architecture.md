...
- BPF filter，用于缩小捕获范围，例如 `tcp`、`udp`、`host`、`port`、`net`。
- 解析 IPv4/IPv6 上的 TCP 与 UDP packet；其他 packet 保留为 `unsupported_packet` 诊断事件。
...
- 从当前桌面端的目标诊断能力演进：先推荐现有 `utun*` pcap 接口，再考虑由 ProtoLens 自己创建和管理 TUN。
...
第一条解密路径优先支持 NSS `SSLKEYLOGFILE`。Chrome、Firefox、curl、Node 等客户端可以把 TLS session secrets 写入 key log 文件；ProtoLens 读取该文件后，用 ClientHello 中的 client random 匹配对应连接，再结合 TCP session 重组和 TLS record 解密器还原 HTTPS application data 明文。

桌面端应引导用户：

- 选择或创建 key log 文件路径。
- 点击桌面端 `Launch Chrome` 前先提示用户确认已关闭现有 Chrome；确认后由后端按平台启动带 `SSLKEYLOGFILE` 的 Chrome；macOS 使用 `open -na "Google Chrome"`，Windows/Linux 使用 Chrome/Chromium 可执行文件候选路径。
- Chrome 使用隔离 user-data-dir，避免已有 Chrome 进程复用导致环境变量不生效。
- 访问目标站点后，在 ProtoLens 的 `SSL Key Log File` 中选择同一个文件。
- live capture 和 PCAP replay 都使用同一套 key log 解析入口。
- UI 上 key log 路径输入应独占一行，文件选择和 `Launch Chrome` 按钮位于下一行，避免长路径和操作按钮互相挤压。

当前实现已完成 key log 文件加载、自动重载、TCP byte stream 重组、TLS handshake/record 解析、TLS 1.3 会话密钥派生，以及 application data 明文输出。已支持 `TLS_AES_128_GCM_SHA256`、`TLS_AES_256_GCM_SHA384` 和 `TLS_CHACHA20_POLY1305_SHA256`。TCP/TLS 明文会通过 event-level 协议分析器继续输出 `http2.frames`，桌面端在 L7 分区展示 HTTP/2 frame 摘要。

UDP/QUIC 路径也复用同一份 `SSLKEYLOGFILE`。`quic.packet` 分析器记录 long-header 连接 ID 信息，用 TLS 1.3 `CLIENT_TRAFFIC_SECRET_0` / `SERVER_TRAFFIC_SECRET_0` 派生 QUIC v1 1-RTT packet protection key，当前支持 AES-GCM key/header protection；解密成功后输出 decrypted QUIC frame metadata 和 plaintext bytes。QUIC STREAM frame 会按 stream 聚合，并继续解析 HTTP/3 frame，输出 `http3.frame` observation。HTTP/3 HEADERS 使用 QPACK 静态表和 Huffman 字符串解码，UI 中显示 method、authority、path、status、content-type、DATA 长度和可读 preview。

当前限制：TLS 1.2 `CLIENT_RANDOM` 尚未实现；QUIC Initial/Handshake/0-RTT 仍只做 packet metadata，不做完整解密；QUIC ChaCha20 packet protection、QPACK 动态表、HTTP body 解压缩和 HTTP/1.1 请求响应聚合仍是后续工作。乱序、重传和大 body 的会话级还原也需要继续加强。
...
- 代理工作流仍可能需要禁用或降级 QUIC/HTTP3；当前 QUIC/HTTP3 支持来自 `SSLKEYLOGFILE` 离线/旁路解密，不等同于 MITM 代理。

### 复用外部代理解密结果

如果用户已经使用本地代理，例如 Clash、Surge、mitmproxy、Charles、Fiddler 或浏览器调试代理，ProtoLens 后续可以通过插件或导入接口消费它们的明文日志、HAR、事件流或 MITM 输出。

该路径不应假设所有代理都能导出明文；实现时需要按具体代理能力做适配，并在 UI 中明确告诉用户当前采集的是“浏览器到代理”、“代理到远端”还是“代理解密后的 HTTP 事件”。
- 工具不应静默安装 root CA。
...
协议 observation 可以由 packet-level 或 event-level 插件产生。packet-level 插件直接消费 `interface.packet`，例如 DNS、TCP、UDP/QUIC metadata；event-level 插件消费已有事件，例如 TLS 明文上的 HTTP/2 parser，或 QUIC decrypted STREAM 上的 HTTP/3 parser。这样新增 L7 协议时不需要改动抓包主流程。
